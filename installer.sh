#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/ciefp/CiefpSRTplayer/main/installer.sh -O - | /bin/sh
...
TMPPATH=/tmp/CiefpSRTplayer

if [ ! -d /usr/lib64 ]; then
	PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpSRTplayer
else
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpSRTplayer
fi
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   wget https://github.com/ciefp/CiefpSRTplayer/archive/refs/heads/main.tar.gz
   tar -xzf main.tar.gz
   cp -r 'CiefpSRTplayer-main/usr' '/'
set +e
cd
sleep 2
...
echo ""
echo "#########################################################"
echo "#          CiefpSRTplayer INSTALLED SUCCESSFULLY        #"
